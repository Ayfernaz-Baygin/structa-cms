...
export default function EditProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    async function loadProject() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data?.message ?? 'Proje yüklenemedi.');
        }
  ...
  return (
    <div>
      ...
      <h1 className="mt-2 text-3xl font-semibold">Projeyi Düzenle</h1>
      <p className="mt-2 text-zinc-400">Proje içeriğini, galerisini ve yayın durumunu güncelleyin.</p>

      <div className="mt-8">
        {notFound ? (
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 px-6 py-16 text-center">
            <p className="text-sm font-medium text-white">Proje bulunamadı.</p>
            <Link
              href="/projects"
